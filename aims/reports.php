        ...
        <?php
            ...
            foreach($events as $ev) {
              ...
              if($evType == "Socio-Cultural") {
                #ipakita ang judges
                $sql = "select DISTINCT (SELECT userId from vw_accounts where userId = personnelId) 
                AS perId, 
                (SELECT firstName from vw_accounts where userId = personnelId) AS perName 
                from sub_results where eventId = ?;";
                $stmt = $conn->prepare($sql);
                ...
                if ($retval->num_rows > 0) {
                  ...
                  while($row = $retval->fetch_assoc()) {
                    ...
                    $pername = $row['perName'];
                    ?>
                      <a href="judge_scoresheet.php?event=<?php echo $eventId ?>&evname=<?php echo $evName ?>&personnel=<?php echo $perid ?>" target="_blank" style="margin-right: 10px;">
              ...
              if($evType == "Sports") {
                ...
                if ($retval->num_rows > 0) {
                  ?>
                  <td>
                    <a href="viewtally.php?event=<?php echo $eventId ?>&evname=<?php echo $evName ?>" target="_blank">View Tally</a>
                  </td>
            ...
            if ($retval->num_rows > 0) {
                ...
                while($row = $retval->fetch_assoc()) {
                    ...
                    $sql = "select DISTINCT (SELECT userId from vw_accounts where userId = personnelId) 
                            AS perId, 
                            (SELECT firstName from vw_accounts where userId = personnelId) AS perName 
                            from sub_results where eventId = $evid;";
                    $prepstmt = $conn->prepare($sql);
                    $prepstmt->execute();
                    $retper = $prepstmt->get_result();
        
                    if ($retper->num_rows > 0) {
                        while($row = $retper->fetch_assoc()) {
                            $prId = $row['perId'];
                            $prName = $row['perName'];
                            ?>
                            <td><button><?php echo "$prName ($prId)" ?></button></td>
                            <?php
                        }
                    }
                    $retper->free();
                ...
                ?>
                <tr>
                    <td colspan="2">No Events and Score Sheet to Show</td>
                </tr>
                <?php
            }
            $retval->free();
